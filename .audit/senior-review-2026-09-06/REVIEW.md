...
### E16 progress — 2026-09-06 evening

Bricks landed, each a props-seam move with `haptic`/`Confetti`/helpers imported from App the way the lazy screens already do: **1** Results (+CountUp, ResultsCloseBtn, WrongAnswersReview, lib/resultsCopy, lib/stump, getXPForResult→lib/scoring) · **2** local pass & play (setup, engine, podium, PlayerMark) · **3** OnlineHubTab · **4** StumpScreen · **5** PrivacyScreen · **6** SettingsScreen (+toggle, InstallCard, about constants) · **7** OnboardingScreen (+preboot helpers) · **8** ModeResults (Hot Streak, True-or-False, resultVerdict). App.jsx **14,027 → 11,370**. Home budget unchanged (same chunk); the gains are TDZ surface and per-screen testability.

⚠️ Two things the day taught. (a) `npx playwright test` targets `BALLIQ_BASE_URL`, default 4173 = the last `vite build`; my `PW_BASE=…5173` was ignored all day, so every spec pass was of the previous build. A deliberate break (unexported `let` after brick 7) stayed green on 4173 and went red on 5173. Every spec run is now against 5173, and the broken-on-purpose check is the proof. (b) `vite build` is the cheap detector for cross-file exports; lint cannot see them.

Remaining in App.jsx: **AppInner 5,342** (the budget), QuizEngine 810 (+TypedInput 74, HardRightBurst 64; needs TIMINGS/norm/checkTyped exported), ClubQuizScreen 204 (club maps), DailyReviewScreen/PuzzleReviewScreen (~260), ScreenLoading 105.

### E16 — end of day 2026-09-06

Bricks 9 and 10 followed (QuizEngine + TypedInput + HardRightBurst; ClubQuizScreen; DailyReview/PuzzleReview/Mini7Strip): **App.jsx 14,027 → 9,898**. Then the off-path screens became lazy chunks (Settings, reviews, club picker, Privacy, Stump, local play), each behind its own TabErrorBoundary + Suspense: **Home eager JS 831 → 768 KB**, budget ratcheted to 800. Two things the split surfaced and fixed: the quit sheet had `role="dialog"` with no `useModalA11y` (the App-wide dialog/hook tally had hidden it), and the review screen's streak line still used a flame emoji. Every brick was rendered on the DEV server (`BALLIQ_BASE_URL=http://localhost:5173`) and the live site was probed after each deploy.

Left in App.jsx: **AppInner ~5,300** (state, effects, services, the screen switch), the club/league maps and question selection (`getQs`, `applySeenFilter`, `shuffle` — a `lib/questionSelect.js` candidate), ScreenLoading, TopicPickerSheet, the small home widgets. The next honest lever is AppInner's effects and callbacks into hooks (`useDailyLoop`, `useAuthGate`, `useNotifications`…), which is a different kind of move — behaviour, not screens — and wants the boot smoke plus a device pass per hook.

### AppInner hooks — started 2026-09-06 evening

Landed: `hooks/useNotificationCenter.js` (bell state, polls, four actions) and `hooks/useScrollAwareTabBar.js` (one effect, one ref). App.jsx 9,747. A side catch from running the e2e set against the dev origin: the consent banner was drawn over the share-name sheet's "Share without a name" for a first-visit EU player; `public/consent.js` now yields to any `[role="dialog"]`, and `tests/e2e/consent-state.json` pre-declines on 5173 too so dev runs mirror CI.

Mapped for next (statement counts from the analyser, AppInner top-level statements per domain): **share** 9 statements / 312 lines (shareScore, saveCardImage, shareProfile, performDailyShare, shareDaily, submitShareName + the name-sheet refs) → `useShare`; **web push** 2 statements / 47 lines → `useWebPush`; **join gate** 17 statements / 277 lines (pendingJoinCode, clearPendingJoin, hubJoinRoom, leave/invite pending states, the boot-time dead-code drop) → `useJoinGate`; **login streak** 31 statements / 1,666 lines — it reaches into `handleComplete` (359) and `saveStats` (168), so it is a behaviour refactor with a device pass, not a move. The analyser is a 40-line python snippet in the session log: statements are `  \S`-indented lines of AppInner; inputs are identifiers declared elsewhere in AppInner.

### AppInner hooks — evening, continued
