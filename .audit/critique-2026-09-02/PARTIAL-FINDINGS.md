# Website critique — PARTIAL HARVEST (2026-09-02)
> ⚠️ **THESE FINDINGS ARE UNVERIFIED.** The fleet hit the session limit after
> 3 of 8 lenses. **Every adversarial skeptic failed**, so the workflow's
> `refuted: 0` means *"nobody checked"*, not *"all findings held"*. Treat each
> as a hypothesis until reproduced. Five lenses never ran at all: iPhone Safari
> (the one Alex asked for by name), strategy/conversion, copy & voice,
> accessibility & performance, and other arrival contexts. Neither graded report
> was written.
**Lenses completed:** 3 of 8 — Desktop design critique (1440x900 and 1920x1080) → **B-**; Mobile design critique — and the Scouting Report conceit → **C+**; SEO surfaces as landing pages (/lists/* and /quiz/*) → **B-**

## Desktop design critique (1440x900 and 1920x1080) — grade B-
*Engine:* webkit 26.5 via Playwright (Safari's engine) for every measurement and screenshot; chromium was available but not used

**Why this grade:** Desktop is a designed experience on the flagship surfaces (homepage, club quiz hero, clubs index) — real two-column compositions that hold at 1440 and 1920 with no horizontal scroll and full focus-visible coverage. It loses a grade for one functional bug (the lists sticky name-bar disappears under the sticky header), stranded mobile components (Footle card with a 552px void, truncated club chips, single-column lists), and two design systems running side by side.

**What works**
- Homepage is genuinely composed for width: hero column 944px, Scouting Report card 1080px with a 2x2 answer grid, Footle section as a 2-col grid (board left, legend + countdown right), clubs list in 2 columns. docH is identical (3271px) at 1440 and 1920, i.e. the layout is stable, not stretched (home@1440-fold-consent.png, home@1920-full-clean.png).
- /quiz/clubs/ is a true desktop page: 238px league sidebar + 874px 5-column club grid + filter input, 65 tappables above the 900px fold (quiz-clubs-index@1920-fold-consent.png).
- /quiz/arsenal/ hero is 2-col: copy + store badges left, a live 4-option question right at y=201-563, so a desktop visitor can play from the fold (clubquiz-arsenal@1920-fold-consent.png).
- Physical keyboard works for Footle on both / and /football-wordle/: typing SALAH landed S-A-L-A-H in the board tiles with no click on the on-screen keyboard (home@1440-physical-keyboard-typed.png, footle@1440-physical-keyboard-enter.png).
- Keyboard focus is visible on 118 of 119 tabbed controls across 10 page/viewport runs (3px outlines); nav dropdowns open on click, positioned correctly (Games menu 236x232 at l534 t65, arsenal@1440-nav-games-open.png); no horizontal scroll and zero page errors on any of the 10 runs.

**Findings (11)**

### [HIGH] DD-1 — Lists page: the sticky 'Type a name…' bar scrolls under the sticky header and becomes unclickable
...
- **User cost:** /lists is 47% of Google impressions and averages 2.3s dwell; the name-them-all game is the one playable thing on the page, and it vanishes for the entire 3,333px table scroll (6.3 folds at 900px) on desktop.
...
- **Evidence:** lists@1440-sticky-under-header.png (input ghosted behind Games/Quizzes/Discover); measured: input top 11 bottom 53, sticky top 0px z 5, header sticky 0-71 z 100, hitAtInputCentre=BUTTON.nav-top
- **Confidence:** measured

### [HIGH] DD-2 — Club quiz 'More quizzes to try' truncates 29 of 134 club names in 116px tiles at 1440
...
- **User cost:** This is the internal-link block on the pages that already rank top-10 for 'X quiz with answers'; on a 1440px screen with 1112px available, Manchester United and Barcelona are unreadable as names.
...
- **Evidence:** arsenal@1440-chips.png; measured tile 116x65, 134 tiles, 29 truncated (scrollWidth > clientWidth), all tiles carry text-overflow:ellipsis/line-clamp
- **Confidence:** measured

### [MEDIUM] DD-3 — Primary CTAs have no hover state on desktop
...
- **User cost:** On a pointer device the only buttons that give no feedback are the ones that start a game or install the app; 48% of new accounts never play, and the desktop CTA reads as a static badge.
...
- **Evidence:** results.json hovers: home 'Play free' b4==af (rgb(6,35,12)|rgb(88,204,2)…) at 1440 and 1920; 'Play today' b4==af; arsenal 'Play free in your browser →' and orange-banner badges in the unchanged list; nav Games changed
- **Confidence:** measured

### [MEDIUM] DD-4 — /football-wordle/ game card is a 560px mobile card stranded in a 1112px column (552px dead void)
...
- **User cost:** Footle is the retention engine (62 of 66 players with 3+ active days came through it) and this is its SEO landing page; on a laptop the playable part occupies 39% of the content width beside a hole.
...
- **Evidence:** footle@1440-card.png, footle@1920-fold-consent.png; measured card left 164 width 560 max-width 560px, wrap width 1112, voidRight 552, hint width 1112, tile 55px, key 48x48
- **Confidence:** measured

### [MEDIUM] DD-5 — Two design systems ship side by side: homepage vs every SEO page
...
- **User cost:** A visitor who lands on /quiz/arsenal/ from Google and clicks the logo arrives on a page with a different header, logo, typeface and footer; on desktop, where both are fully visible, this reads as two sites.
...
- **Evidence:** home@1440-fold-consent.png vs clubquiz-arsenal@1920-fold-consent.png; hdr_home/hdr_seo JSON (logoFont Archivo Narrow vs Inter, position static vs sticky, bg transparent vs rgba(10,10,10,.82), links 5 vs 6); h1 fonts from results.json; focus outline colours from results.json focus arrays
- **Confidence:** measured

### [MEDIUM] DD-6 — /lists pages are a single 760px mobile column at desktop: 87-96 characters per line, 60% of a 1920 screen empty, 8.8 folds tall
...
- **User cost:** This is the content type with 47% of impressions and 4% of clicks and 2.3s dwell; on a laptop it presents as a phone page pasted in the middle of the screen, with over-long lines on the only prose.
...
- **Evidence:** lists-ballondor@1920-fold-consent.png, lists@1440-table.png, lists-ballondor@1440-full-clean.png; measured col left 340/580 width 760, cpl samples 96/90/87 at 16px, table 758x3333 rows 70, docH 7886, foldsToTableEnd 6.3, taster buttons 726x52
- **Confidence:** measured

### [LOW] DD-7 — Footle hero subtitle orphans 'up.' on its own line at both 1440 and 1920
...
- **User cost:** Cosmetic, but it is the first sentence on the Footle landing page and the hyphen split 'sign-\nup.' reads like a typo on a large screen.
...
- **Evidence:** footle@1920-fold-consent.png, footle@1440-full-clean.png; measured p width 623 max-width 623.29px fontSize 19px lines 2 lastLineW 28; .fw-foot width 1112 chars 286 lines 2 cpl 143 fontSize 13.5px
- **Confidence:** measured

### [LOW] DD-8 — Clubs-index filter input has no visible keyboard focus indicator
...
- **User cost:** A keyboard user tabbing into the page (first stop after the nav) cannot tell that the filter is focused before they start typing.
...
- **Evidence:** clubs@1440-input-focus.png (no ring); measured afterKb fv=true, outline none 3px rgb(255,255,255), boxShadow none, border unchanged
- **Confidence:** measured

### [LOW] DD-9 — Orange 'Prove it' banner uses a 343px text column inside a 1112x432 banner
...
- **User cost:** The strongest conversion block on the club pages reads as a phone card blown up; the copy wraps where it has 3x the room.
...
- **Evidence:** arsenal@1440-cta.png; measured banner 1112x432, innerW 343, three text nodes width 343 each at 2 lines; consent overlap from results.json coveredByConsent ['Play free in your browser →@882-933'] vs consent top 823
- **Confidence:** measured

### [LOW] DD-10 — 1920 gets the 1440 layout centred: no fluid type or wider container
...
- **User cost:** On a 1920 monitor the page is legible but small; the dark background hides the void, so this is the least costly of the width issues.
...
- **Evidence:** home@1920-fold-consent.png; measured h1 64px at both, home column 944 left 248/488, heroVoidR 856 at 1920, arsenal container 1112 left 404, lists column 760 left 580
- **Confidence:** measured

### [LOW] DD-11 — Known consent-bar coverage, confirmed at desktop widths (not a new finding)
...
- **User cost:** Adds depth to the already-open item; desktop cost is bounded to the last 77px of the fold.
- **Fix:** Already tracked; on desktop the bar could be a 56px single-row variant (text 13px + two 36px buttons) to reclaim 21px.
- **Evidence:** *-fold-consent.png for all 10 runs; results.json consentUp.consent and coveredByConsent per run
- **Confidence:** measured

**Could not test**
- Real Safari.app on macOS (Playwright WebKit shares the engine but not Safari's UI chrome, overlay scrollbars or the exact system-font fallback); all numbers are WebKit 26.5.
- Retina (devicePixelFactor 2) rendering at 1440/1920 — screenshots were taken at DPR 1.
- Widths beyond 1920 (2560 ultrawide) and the 1024-1280 laptop band.
- Localised pages (/es/, /nl/ …) and the /lists/ index page at desktop — only one lists detail page was measured.
- Logged-in app screens (Daily, Profile, Friends, Results) — out of the marketing-site lens.
- Actual pointer hover on the nav dropdowns beyond click (hover did not open them; whether that is intended was not verifiable).

## Mobile design critique — and the Scouting Report conceit — grade C+
*Engine:* webkit (Playwright webkit-2336, Safari's engine). Device descriptors iPhone 11 Pro = 375x635 content viewport of the 375x812 screen, and iPhone 12 Pro = 390x664 of 390x844 — i.e. the fold numbers below are Safari's real content area with its chrome subtracted, not the raw screen height. Live https://balliq.app/ on 2026-09-02.

**Why this grade:** The craft fundamentals are measurably strong (every text ≥5.15:1, every primary target ≥44px, one saturated hue in the fold, a 102px verdict payoff), so this is not a broken page. It is held to C+ by two measured defects on the core loop — every "Next question" tap lands with the new question 187px above the viewport, and a first-time visitor sees zero answer options — plus a flat headline scale and a 210px empty ledger that the conceit does not earn before play.

**What works**
- Contrast discipline is real, not claimed: a census of every text node inside .sr found nothing under 4.5:1; the floor is the lede/footer grey rgb(126,130,140) on rgb(10,10,10) at 5.15:1 (hand-verified), ink-on-paper is 14.71:1, the verdict rust rgb(148,71,42) on paper is 5.38:1.
- Touch targets: nav buttons 44px, Play free 108x44, answer options 273x52, Next question 273x52, club rows 44px, summary/store badges 48px. The only sub-44 targets on the page are the Footle keyboard keys (27.4x48, a keyboard convention) and the consent bar's own Privacy policy link (85.7x15.7).
- Colour hierarchy is correct in the fold: exactly one saturated hue, rgb(88,204,2), on two elements (the IQ mark and Play free); page-wide there are only two green buttons (y=14 and y=2054). The eye lands on Play free, then drops to the pale paper — the paper is the only light surface on a 4,137px page (16.2:1 luminance step vs the desk), so the question is where the eye goes second. That is the right order.
- The verdict payoff is the best screen on the page: a 102px score in a tier-coded colour, a 34px tier name, and the five disciplines filled in as 0 of 1 / 1 of 1 in red/green (375x812-11-verdict-slab.png). The dossier conceit fully earns itself here.
- Option rows are well built: 52px, A–D key glyphs, filled-ink Next (the only filled button on the paper), and the wrong/right marks use the same ink-red/ink-green as the ledger, so the language is internally consistent (375x812-06-slab-after-answer.png).

**Findings (9)**

### [HIGH] DM-1 — Every 'Next question' tap lands with the new question above the viewport
...
- **Evidence:** 375x812-15-viewport-after-next-q1.png, 375x812-17-after-next-q1-settled.png (both show 'Inter / Napoli' at the top with no question), m2-flow.json (qTop −187/−162/−162/−162), audit4 output (t+300/1000/3000 all scrollY 660, qTop −187).
- **Confidence:** measured

### [HIGH] DM-2 — First-time visitor sees zero answer options: 133px mast + 172px consent bar squeeze the question out of the fold
...
- **Evidence:** 375x812-01-fold-firstvisit.png, 390x844-01-fold-firstvisit.png (only 'Premier League' visible above the bar), measure-375x812-firstvisit.json (consent y=463.4 h=171.6; .sr-q y=473.1 h=87.5; .sr-opt y=574.5; .sr-mast h=133), measure-390x844-firstvisit.json (consent y=492.4).
- **Confidence:** measured

### [MEDIUM] DM-3 — Games dropdown is 105px wide and its labels spill over the headline
...
- **User cost:** The first thing a curious visitor taps in the nav renders as a broken overlay: four of five labels overflow, and the one that names Footle — the retention engine — is the worst clipped. It reads as a bug within one tap of arrival.
...
- **Evidence:** 375x812-08-nav-games-open.png; audit3 output: drop items [label, clientWidth, scrollWidth] = ['Footle — football Wordle',89,174], ['Mystery Player',89,110], ['Transfer Trail',89,97], ['Guess the XI',89,97], nowrap, overflow visible; m2-navopen.json (.sr-drop w=105.2, item h=41).
- **Confidence:** measured

### [MEDIUM] DM-4 — The Scouting Report conceit: keep the letterhead, drop the five-row 'not assessed' ledger before play
...
- **Evidence:** 375x812-05-report-slab.png, 375x812-03-fold1-firstvisit.png, 375x812-11-verdict-slab.png; measure-375x812-firstvisit.json (.sr-file h=736.5, .sr-stubwrap y=846.5 h=209.5, .sr-stub h=164.5, rows 32.9px, .sr-out 13.5px 6.6:1, .sr-lh h=60.7); m2-base.json (.fb y=1132); m2-verdict.json (scrollY 466, .sr-score y=402 h=75.5 after the table).
- **Confidence:** measured

### [MEDIUM] DM-5 — Headline scale is flat: h1 35px vs three h2s at 32px, all uppercase Archivo Narrow
...
- **User cost:** The hero does not announce itself as the hero on the device where 100% of the fold is vertical; the reader scanning down meets three more 'headlines' of the same voltage and the page loses the dossier's single-subject feel. The 12–18px wobble means labels, metas and body sit at half-steps that read as inconsistency rather than a scale.
...
- **Evidence:** measure-375x812-firstvisit.json (.sr-h1 35px/33.25px; .sr-h2 32px/31.36px), m2-base.json (.fb-h2 32px; typeCensus 17 size/weight combos), report.css line 5 (--ty-headline clamp(35px…), --ty-section clamp(32px…)), 375x812-02-full-firstvisit.png.
- **Confidence:** measured

### [MEDIUM] DM-6 — Verdict has no primary action: two identical outline buttons at the payoff
...
- **Evidence:** 375x812-11-verdict-slab.png, 375x812-12-verdict-fold-top.png; m2-verdict.json (.sr-a and .sr-webbtn both 303x52, color rgb(20,23,26), transparent bg; allCtas shows only two green buttons on the whole page at y=14 and y=2054.7).
- **Confidence:** measured

### [LOW] DM-7 — Two-row mast eats 21% of the fold
...
- **User cost:** Directly feeds DM-2: the 60px second row is the difference between the first answer option clearing the consent bar or not at 390 wide. Also unusual for mobile — a single 56–64px bar is the norm and the dropdowns already exist to hold the groups.
...
- **Evidence:** measure-375x812-firstvisit.json (.sr-mast h=133; .sr-nav y=74 h=44; .sr-h1 y=155), 375x812-04-fold-dismissed.png.
- **Confidence:** measured

### [LOW] DM-8 — Footle band is 1,430px tall and explains the game after asking you to play it
...
- **User cost:** A taster that is meant to be tried in seconds spends more than two screens before its own CTA and puts the rules after the button. The blue-cast surfaces on a neutral page are a small but measurable palette inconsistency (three dark greys: 10/10/10, 18/20/27, 20/22/30).
...
- **Evidence:** 375x812-07-footle-band.png, 375x812-03-fold2-firstvisit.png, 375x812-03-fold3-firstvisit.png; m2-base.json (.fb y=1132 h=1429.7 bg rgb(20,22,30); .fb-real.fb-primary y=2054.7; .fb-legend y=2236.7 h=273; .fb-six y=2331.7; .fb-time 44px; sectionBgs; .sr-drop bg rgb(18,20,27)); audit4 output (keys 27.4x48, gap 5px, tiles 36px).
- **Confidence:** measured

### [LOW] DM-9 — Answering with the consent bar up hides the explanation and Next (confirms the known class on the homepage)
...
- **User cost:** The known 'consent covers Next question' item applies to the homepage taster too, not only club quizzes; here it also hides the Why panel, which is the feature the clubs section brags about ('most of them tell you why').
...
- **Evidence:** 375x812-16-consent-up-after-answer.png; audit3 output: next top 667 bottom 719, consentTop 463, vh 635, scrollY 284.
- **Confidence:** measured

**Could not test**
- Real iOS Safari chrome behaviour: the URL bar collapses on scroll, which would grow the 635/664px content area by ~100px mid-session; Playwright's descriptor holds it fixed, so fold numbers are the conservative (bar-visible) case.
- Finger-driven momentum scroll and rubber-banding; all scrolls here were programmatic or click-induced. DM-1 was stable at 300ms/1s/3s, but a real thumb flick that overshoots was not simulated.
- Dynamic Type / iOS font-size accessibility settings, and the actual Archivo/Archivo Narrow rendering on a physical display (the fonts loaded and were measured by family name, but not inspected for hinting).
- The desktop-only −0.45deg sheet rotation and twin-sheet shadow (≥1140px) — out of this lens.
- The 'Skip to the assessment' link (positioned at x=−9999) was not keyboard-tested; VoiceOver reading order of the ledger table was not tested.
- In-app browsers (TikTok, Instagram) were deliberately not measured per the brief.

## SEO surfaces as landing pages (/lists/* and /quiz/*) — grade B-
*Engine:* webkit (Playwright WebKit, iPhone 13 descriptor 390x664 CSS px, DPR 3, fresh context per page = first visit with consent bar up; desktop pass at 1440x900 WebKit). Every number below is WebKit unless marked otherwise.

**Why this grade:** The /quiz/ club pages are genuinely strong landing pages (static HTML, question and 4 answer options inside the first 664px, 145-150ms tap feedback, Quiz schema, hreflang) and the localised ones match them. The /lists/ pages have a real playable taster, but the thing the searcher came for sits 4.1-4.3 folds down behind a 1,972px quiz block, the table is 66-139px wider than the phone with the key column cut off and no scroll hint, and there are zero localised lists.

**What works**
- Every sampled page is static HTML: the raw response carries the real <title> and <h1> (e.g. raw <title> 'Arsenal Quiz with Answers — Gunners | Ball IQ'), so nothing depends on the SPA booting. First-contentful-paint 407-1,888ms across 10 pages; load event 1.2-2.4s.
- Club pages put the taster IN the first fold: /quiz/arsenal/ question at y=402 and options A-D at y=467/524/581/638 on a 664px viewport (quiz_arsenal__fold_nobanner.png). Liverpool 491-663, Premier League 541-712. Tapping an option changes the DOM in 149ms with correct/incorrect marks and no sign-up (quiz_arsenal after-tap in sections.log).
- Localised club pages mirror the English pattern exactly: /es/quiz/river-plate/ first option at y=468, /tr/quiz/galatasaray/ at y=491, both with 3 hreflang links and a 'PREGUNTA 1 / 10 · 0 correctas' live score chip (es_quiz_river-plate__fold_consent.png). Tap feedback 480ms on River Plate.
- /lists/ pages are NOT text-only: each carries a 5-question tappable taster (23 option buttons) that responds in 144-149ms (lists_ballon-dor-winners__after_tap.png), plus ItemList schema with 70-73 ListItems, a 'Play — hide the answers' name-guessing game that actually blurs the names (lists_ballon-dor-winners__after_play.png), and a type-to-find search input.
- No horizontal page overflow on any of the 10 pages (documentElement.scrollWidth <= 390); the only overflow is inside the table wrapper (overflow-x:auto).

**Findings (6)**

### [HIGH] SEO-1 — On /lists/ the thing the searcher came for is 4.1-4.3 folds down, behind a 1,972px quiz block
...
- **Evidence:** lists_ballon-dor-winners__full_consent.png (full page), lists_ballon-dor-winners__table_region_y2600.png, overflow.mjs output: tasterTop 637 / tasterH 1972 / tableTop 2829; sections.log MAIN CHILDREN y=637 h=1972 then y=2609.
- **Confidence:** measured

### [HIGH] SEO-2 — List tables are 66-139px wider than the phone; the answer column is cut off with no scroll hint
...
- **User cost:** The table looks complete (it ends cleanly at the card edge in lists_ballon-dor-winners__table_region_y2600.png) so a reader has no cue that a fourth column exists; on the Golden Boot page the goals tally is effectively invisible on an iPhone.
...
- **Evidence:** overflow.mjs output: wrapW 350, tableW 489/477/456/416, hiddenPx 139/127/106/66, ths left/right coordinates, scrollHint false x4; lists_ballon-dor-winners__table_region_y2600.png shows YEAR/WINNER/CLUB(S) only.
- **Confidence:** measured

### [MEDIUM] SEO-3 — Zero localised /lists/ pages exist, while 46 localised /quiz/ pages do
...
- **Evidence:** urls.txt from sitemap.xml (grep counts 51 / 0 / 46); curl status lines in the localised-lists check (404 x3).
- **Confidence:** measured

### [MEDIUM] SEO-4 — First-visit fold on /lists/ contains no tappable football at all; the consent bar covers y=492-664 on top of that
...
- **User cost:** Nothing to do in the first screen on a 2.3s-dwell page type.
...
- **Evidence:** lists_ballon-dor-winners__fold_consent.png, lists_world-cup-winners__fold_consent.png; results.json taps (first non-nav tappable y=909/835/886/810); consent rect top 492 h 172.
- **Confidence:** measured

### [LOW] SEO-5 — Name-game chrome ('Type a name… · 0 found · Give up') is shown before anyone presses Play
...
- **User cost:** A reader who only wants the list sees a scoreboard for a game they did not start and a 'Give up' button with nothing to give up on; on a 664px viewport with the 67px nav and 64px sticky bar, 131px (20%) of every table screen is chrome.
...
- **Evidence:** lists_ballon-dor-winners__table_region_y2600.png (pre-Play: input + '0 found' + 'Give up' shown, names visible) vs lists_ballon-dor-winners__after_play.png; overflow.mjs liveBarBeforePlay.
- **Confidence:** measured

### [LOW] SEO-6 — The 47% / 4% ratio is mostly SERP-side (rank and head terms), and the page only decides what a click is worth
...
- **User cost:** Treating /lists as a landing-page-design problem alone will not move the click count; treating it as a rank problem alone will not move dwell. Both halves need the specific fixes above.
...
- **Evidence:** results.json paints/timings for 10 pages; prior GSC figures from memory file project_lists_content_type.md (labelled as prior, not re-measured).
- **Confidence:** inferred

**Could not test**
- Search Console per-page position/CTR could not be re-measured this session: sc_query_performance_tool returned entitlement_required on the AdvisorPPC account. Prior 2026-08-15 GSC figures are quoted from memory and labelled as such.
- Real time-to-first-interaction at DOMContentLoaded: my DCL-tap harness had ~1.3-1.7s of its own overhead before the tap fired, so I can only assert that taps work by the load event (1.2-2.4s) and respond in 144-480ms; I cannot claim the exact earliest millisecond a tap works.
- Real Google SERP appearance (featured snippet / PAA presence) for the sampled queries was not fetched this session.
- In-app browsers (TikTok/Instagram) were deliberately not measured for this lens, per the brief.
- Whether tapping 'Allow' on the consent bar changes any layout metric (only 'Decline' was exercised, as the privacy-preserving choice).
