...
- [ ] **🔴 MP PLAYTEST BUGS (Alex vs Johannes, 2026-08-03) — playtesters are ~100% precision, treat all four as real:**
  1. **H2H record is DATA-wrong, not just label-wrong — triangulated from both devices.** Alex's phone: "You lead 3–0 vs Johannes" right under "Johannes Jahr Hauger wins" (true record 2–1 to Alex). Johannes's screen: "Alex leads 3–0 vs Alex". Both agree on "3–0 to Alex" → the tally attributes ALL finished rooms between the pair to one player (creator / pair[0]?) instead of counting winners; 3 games = 3–0. Separately, the guest device renders that player's name in BOTH slots (no self/opponent resolution). Start: the H2H query/RPC winner attribution, then the banner's name mapping. ⚠️ rpc() resolves on error, never throws.
