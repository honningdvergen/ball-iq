...
getting lower and did not reflect how well he answered"; History compact and
its ↺ cells not obviously tappable, "I believe we agreed people were not supposed
to replay previous days"; encapsulate the four dailies?; multiplayer dull —
Online or Multiplayer?; and "Good afternoon, Alex" would not fit a long name.
Decision (13:00): do all but the encapsulation. Five-list first, then the
seven-list (reach → card-as-share → club-page↔app → authority → question loop
→ /nl/ → calibration-tests-as-practice).

- [x] **1. THE SCORECARD — e90d4fe, gate green, browser-verified at 375, NOT PUSHED
      (Alex tests first; every number moves).** Three defects + a miscalibration:
      the overall averaged unplayed faces at an invented prior; that prior was
      the player's own hot start (the friend's decline, reproduced: 86→78 over
      35 answers at 65%); and HALF OF ALL ANSWERS never reached the card —
      club quiz re-tags rows cat:"ClubQuiz" (the biggest bucket in prod, 2,039
      answers) and it fed no face. Scale 40+59·acc put the measured MEDIAN
      (acc 0.58, n=104) at 74 and a third of players in gold.
      Now: overall = every answer shrunk to the population median (weight 20 —
      weight 10 still slid a lucky 8/10 from 79 GOLD to 71 SILVER); scale from
      measured anchors (p50→65, p75→75: gold = top quarter), generated by
      `scripts/calibrate-card.mjs` → `src/data/cardCalibration.js`; club
      answers carry realCat+club and route to faces by league (`CLUB_NAME_TO_COMP`,
      generated, 96/96); ChampionsLeague→UCL, Euros→INT; faces print only from
      their own data; one-time "Ratings recalibrated" note on the card.
      Simulated: friend 73→70 no tier flip; steady 67% player climbs 69→74.
      ⚠️ Found on the way: `.pd-*` rules at app.css:3035+ sit INSIDE
...
an array hole from a stray comma had crashed it; a probe file with a known TDZ is the proof. Follow-ups the same afternoon: a
zero score is no longer green; the Survival header's 🔥/🏆 are Lucide; four stale e2e specs rewritten to today's product
(Footle entry via the Today row, local mode via the Online tab, Daily tab = History, onboarding reload accepts the browser
front door — bare /play in a browser goes to `/` by the 09-05 decision). Native build 22 on the simulator: Survival Q1 death
renders the soft landing + one primary + quiet row. **NEXT:** E16 (split App.jsx
...
### K. Bloodhound sweep 2026-09-07/08 — `.audit/bloodhound-2026-09-07/REPORT.md` (9 finders; 22 findings still UNVERIFIED there)
✅ **[block] D14 IS NOT MET — the budget instrument never walks GameRoot's static imports.** 901 KB blocking (supabase 211 KB static, FootballWordle 33, trail 23), not 593. Fix the instrument first, then the number, then decide whether the Supabase client can be deferred for guest first visits.
✅ **[wrong] Day-2 email decides 'yesterday' and 'today' on the server's UTC date — the defect v1_8 documented and fixed for reminders, re-shipped for email** v2_1 applied — `supabase/migrations/v1_12_email_campaigns.sql:63`. The real condition is 'a full player-day has passed since signup and they have not played in it'. Without any tz signal for this audience, stop using calendar dates altogether and use elapsed time, which is timezone-free: select signups with created_at between
✅ **[wrong] Club-quiz reveal scroll clears a 64px button that now rides 190px higher when the consent bar is up** 3481bfa — `scripts/seo/club-quiz-engine.js:321`. Make both numbers read the same variable the button already reads. In scripts/seo/quiz-widget.mjs:93 set `.bq-why{scroll-margin-bottom:calc(76px + var(--biq-consent-h, 0px))}`; in scripts/seo/club-quiz-engine.js:321 compute `var ch=parseFloat(getComputedStyle(
✅ **[wrong] Rate prompt counts any Mac desktop click as an Apple store tap — 'Macintosh' stands in for 'has an App Store to review in'** 3481bfa — `src/App.jsx:8386`. Make the rate sheet use the one store detector the repo already has — `detectStore()` / `isIOSUA()` from src/components/StoreBadge.jsx (Mac counts as iOS only when navigator.maxTouchPoints > 1, i.e. iPadOS desktop-mode Safari) — so 'ios' → apple tap + write-re
✅ **[wrong] Club badge code disagrees between the app and the generated club pages (Barcelona FCB vs BAR, AC Milan ACM vs MIL)** 83a497b — `scripts/gen-seo-pages.mjs:279`. Make the badge code a single generated fact: extend scripts/gen-club-index.mjs (which already emits CLUB_PACK_ABBR from App.jsx CLUB_ABBR on every build) to also emit a slug-keyed map (via CLUB_SLUG_TO_PACK), have scripts/gen-seo-pages.mjs and scripts/seo/leag
✅ **[wrong] Five club colours on the generated pages differ from the app's CLUB_PACKS the map claims to mirror (Norwich yellow vs green, Derby/Swansea white vs black)** 83a497b — `scripts/gen-seo-pages.mjs:337`. Make the web map derived, not typed: have scripts/gen-seo-pages.mjs build CLUB_COLOR from the already-generated src/data/clubPackColours.js (gen-club-index.mjs runs from App.jsx on every build, so the ordering already exists) joined through scripts/club-alias.
✅ **[wrong] forge-skeptic-verdicts cannot reach the one forge that still has the banned line** 3481bfa — `tests/unit/forge-skeptic-verdicts.test.js:6`. Two edits, in one commit. (1) scripts/wave-o-topup-b1.workflow.js:363-365 — make the skeptic branch mirror the examiner branch at :344-345: `if (v.verdict === 'reject') throw ...; const q = v.verdict === 'fix' && v.fixed ? { ...item.q, ...v.fixed } : item.q; r
⬜ **[wrong] 'The reminder hour' is defined twice: the client's per-player median (shown as 'Reminder set for HH:00') vs the server cron's hard-coded 19:00** — `src/lib/playHour.js:24`. Make the server the single owner of "when": extend register_device_token (and the web_push_subscriptions upsert in webpush.js) with a `reminder_hour smallint` that the client sends from getReminderHour() on every registration/reschedule, and change the cron's 
✅ **[risk] daily-done pins THREE copies of the same hasPlayed line as the requirement** 3481bfa — `tests/unit/daily-done.test.js:112`. In tests/unit/daily-done.test.js:112, replace the `.length).toBe(3)` count with an assertion about the INVARIANT: every `hasPlayed`/`played` definition in ProfileScreen.jsx must derive from the ten-answer threshold, e.g. `expect(PROFILE.match(/const (hasPlayed
✅ **[risk] lists-headline-truth pins a wrong season resolution (1999-00 -> 1900) as expected, and its justification is false** 3481bfa — `tests/unit/lists-headline-truth.test.js:78`. In tests/unit/lists-headline-truth.test.js make endYear century-aware — after `const y = Number(season[1] + season[3])`, return `y + 100` when `y < Number(season[1] + season[2])` (the second pair rolled over the century) — then flip line 78 to `expect(endYear(
✅ **[wrong] Club badge drift** FCB/ACM (app) vs BAR/MIL (site) — one source + a test.
✅ **[risk] Dead `import { FootleHero }`** App.jsx:69 — remove.
✅ **[risk] `lists-staleness.mjs` runs without `--strict`** in the build (twice) — add the flag.
❌ Refuted: FootleHero colour-blind share text — component unmounted; moot.
❌ Refuted: api/c.js loop-hit "dead write" — the row count went 0→1 on the first /c/ hit; the write works (the unread status is still a hygiene nit).
✅ **[wrong] Footle's window keydown has no dialog/input guard: Enter on a sheet button submits a guess and cancels the button** 1099b61 — `src/games/FootballWordle.jsx:293`. Right fix: give the window keydown listener in src/games/FootballWordle.jsx:291-297 a "something modal is on top" guard shared with the HotStreak/TrueFalse advance listeners. The cleanest source of truth already exists: src/useModalA11y.js 
✅ **[wrong] Share-name sheet: Escape and backdrop tap SHARE (OS share sheet / clipboard write + 'share-daily-done' row)** 1099b61 — `src/hooks/useShare.js:96`. Give the sheet a real cancel. In useShare.js add `const cancelShareName = useCallback(() => { askShareNameRef.current = false; setAskShareName(false); loopEvent("share-daily-cancel", { via: "name-sheet" }); }, [])` and wire it to useModalA1
✅ **[wrong] Rate prompt: Escape/back-gesture dismissal is not logged as 'rate-prompt-dismissed' while backdrop and 'Maybe later' are** 1099b61 — `src/App.jsx:8020`. Log the dismissal in the one place all keyboard/back exits already converge: the onClose handed to useModalA11y at src/App.jsx:8020 — e.g. keep rateView in a ref (rateViewRef.current = rateView) and pass a STABLE useCallback `() => { loopEv
✅ **[wrong] audit-home-budget.mjs cannot see GameRoot's static import graph — real blocking Home JS is 902 KB, not the 593 KB D14 rests on** 930108d — `scripts/audit-home-budget.mjs:46`. Make the gate measure what the browser actually blocks on: replace the `eager` set (index.html links + the GameRoot file) with the transitive closure of `imports` from `dist/.vite/manifest.json` starting at the entry and at the GameRoot chu
✅ **[wrong] scripts/acct-funnel.sql section 2 still reads the retired event name `acct-first-play`; nothing has written it since 2026-09-07** 1099b61 — `scripts/acct-funnel.sql:66`. In scripts/acct-funnel.sql section 2, make ord 4 match BOTH names — `f.event in ('acct-first-play','acct-game-reached')` (old rows were always arrivals, exactly what acct-game-reached means, so unioning them is honest) — and add a 6th step 
✅ **[wrong] shell.mjs classifies the clubs directory as `/quizzes/` — a path that 404s — so the `store-out` page label 'directory' can never fire and the real directory (/quiz/) files as 'club'** (/quiz/ exact match) — `scripts/seo/shell.mjs:108`. In scripts/seo/shell.mjs:108 move the exact-match tests BEFORE the prefix tests and point them at pages that exist: `P==='/quiz/'||P==='/football-quiz/'?'directory':P==='/'?'home':P.indexOf('/quiz/')===0?'club':...`. Also add the raw `path:
✅ **[risk] head() inlines the club-quiz engine CSS (BQ_CSS, 14.5 KB) into every generated page — 155 of 352 pages have no quiz engine** 83a497b — `scripts/gen-seo-pages.mjs:1716`. Gate BQ_CSS on whether the rendered BODY actually contains the widget, not on page type: build the body string first, then call head({..., bq: body.includes('class="bq')}) (or have renderQuizSet set a flag) and interpolate `${bq ? BQ_CSS : 
⬜ **[risk] app.css ships every lazy-split screen's CSS with GameRoot on boot — 30.9 KB minified of rules only ProfileScreen/LocalPlay/Results/Online/Settings reference, plus 28.9 KB no file references at all** — `src/app.css:3044`. Two separate items; only the second is new. (1) The ~28.9 KB dead half is already TODO m8 (docs/TODO.md:5695) — fold this finding's list (.og-breakdown*, .tactics-*, .iq-hist-*, .fix-*, .ffh-*) into that line rather than opening a new one. 
✅ **[risk] Orphan gate silently skips every sitemap URL that has no dist/ file (52 of 313) and still prints ✅** 1099b61 — `/Users/alexanderbrynolsen/ball-iq/scripts/audit-orphan-pages.mjs:64`. Seed `inbound` from the sitemap, not from dist: at scripts/audit-orphan-pages.mjs line 64 build the map from `indexed` (every sitemap loc, EXEMPT aside) so a URL with no dist file is still a row that must be linked, and make the ✅ line prin
✅ **[risk] CI's 'hard gate' job claims to mirror npm run build but runs none of the 13 audit/gen gates** 1099b61 — `/Users/alexanderbrynolsen/ball-iq/.github/workflows/ci.yml:3`. Make the CI job run the one command Vercel runs — replace the Lint/Build/Generate steps with a single `run: npm run build` (it already chains eslint on src+engine, vitest, the audits, vite build, gen-seo-pages, sw-precache, staleness, serp/
...
### M. Head-term diagnosis + club title test (2026-09-09)
⏳ **Club title test** — arsenal / real-madrid / manchester-city retitled ("Play the X Quiz — Free, with Answers"); liverpool / chelsea / everton are CONTROLS. Baseline in memory `project_club_title_test_2026_09_09`. READ 2026-09-23.
📝 **"football quiz" — the target is `/quiz/`, BY DESIGN.** I first read `/football-quiz/` (0 inbound links, canonical → /quiz/) as an orphan bug; gen-seo-pages.mjs:4054-4098 records the 2026-08-16 decision — two pages cannibalised the term, /quiz/ won on the merits, canonical flipped deliberately, reversible only with data. Stands. (One stale claim there: "it is linked in the nav" — it is not; nav says "All quizzes → /quiz/". Harmless.)
✅ (1cc22d8, verified in dist: 347 × "Football quizzes") **The cheap lever left on the head term: ANCHOR TEXT.** ~350 pages link /quiz/ with the label "All quizzes" — the phrase we want it to rank for appears in none of them. Rename the nav label (and the front-door link) to "Football quiz" / "Football quizzes" so the exact phrase reaches the target from every page. 28d: the family (556 impr, pos 12.7) lands on /quiz/premier-league/ (219) because most variants are PL-phrased — correct; the BARE term (51 impr, 0 clicks) is where /quiz/ must climb, and memory has it ~39-41.
⬜ Authority remains the ceiling for the bare term (JetPunk/Sporcle/FourFourTwo are indexes with years of links). Levers are Alex's: Reddit voice-as-fan, Play Store live, PR. Re-read the bare term on 2026-09-23 with the title test.
